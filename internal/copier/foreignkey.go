...
func (fkm *ForeignKeyManager) buildConstraintDefinition(fk *ForeignKey) string {
	...
	fmt.Fprintf(&builder, "ALTER TABLE %s ADD CONSTRAINT %s ",
	...
	fmt.Fprintf(&builder, "%s (", utils.QuoteTable(fk.ReferencedSchema, fk.ReferencedTable))
	...
	if fk.OnDelete != noAction {
		fmt.Fprintf(&builder, " ON DELETE %s", fk.OnDelete)
	}
	if fk.OnUpdate != noAction {
		fmt.Fprintf(&builder, " ON UPDATE %s", fk.OnUpdate)
	}
